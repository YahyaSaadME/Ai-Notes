...
const Notespage = () => {
  ...
  useEffect(() => {
    ...
    const initialWorkflowStatus = params.get('workflowStatus') || '';
    const initialVisibility = params.get('visibility') || '';
  ...
  const openEditModal = (note: Note) => {
    ...
    setNewNote({
      ...
      workflowStatus: note.workflowStatus || "backlog",
      visibility: note.visibility || "org",
            ...
            {filteredNotes.map((note) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  <span className="bg-gray-100 text-gray-700 px-2 py-1 rounded text-[11px] uppercase tracking-wide">
                    {(note.workflowStatus || "backlog").replace("_", " ")}
                  </span>
                  <span className="bg-gray-100 text-gray-700 px-2 py-1 rounded text-[11px] uppercase tracking-wide">
                    {note.visibility || "org"}
                  </span>
                  {note.assignedTo && (
                    <span className="bg-gray-100 text-gray-700 px-2 py-1 rounded text-[11px]">
                      {note.assignedTo}
                    </span>
                  )}
                </div>
                  ...
                  <div className="flex-1 overflow-y-auto">
                    <div className="mb-6">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <span className="px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-700 uppercase tracking-wide">
                          {(selectedNote.workflowStatus || "backlog").replace("_", " ")}
                        </span>
                        <span className="px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-700 uppercase tracking-wide">
                          {selectedNote.visibility || "org"}
                        </span>
                      </div>
                      ...
                      <div className="mb-6">
                        ...
                        <p className="text-sm text-gray-900">{selectedNote.assignedTo || "Unassigned"}</p>
                      </div>
          ...
          <div className="fixed inset-0 bg-white text-gray-800 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl max-w-md w-full">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <select
                    value={newNote.workflowStatus || "backlog"}
                    onChange={(e) =>
                      setNewNote((prev) => ({
                        ...prev,
                        workflowStatus: e.target.value as "backlog" | "in_progress" | "review" | "done",
                      }))
                    }
                    className="px-3 py-2 border border-gray-200 rounded-lg "
                  >
                  ...
                  <select
                    value={newNote.visibility || "org"}
                    onChange={(e) =>
                      setNewNote((prev) => ({
                        ...prev,
                        visibility: e.target.value as "org" | "private",
                      }))
                    }
                    className="px-3 py-2 border border-gray-200 rounded-lg "
                  >
                ...
                <input
                  type="text"
                  placeholder="Assign to person name or email"
                  value={newNote.assignedTo || ""}
                  onChange={(e) =>
                    setNewNote((prev) => ({ ...prev, assignedTo: e.target.value }))
                  }
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg "
                />
